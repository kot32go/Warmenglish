...
public class HibernateUtil {
	public static User getCurrentTeacher(Session session, int t_id) {
		return (User) session.createQuery("from User u where u.id=" + t_id)
				.list().get(0);
	}
	...
	public static Student getCurrentStudent(Session session, int s_id) {
		return (Student) session
				.createQuery("from Student s where s.id=" + s_id).list().get(0);
	}

	public static Group getCurrentGroup(Session session, int g_id) {
		return (Group) session.createQuery("from Group g where g.id=" + g_id)
				.list().get(0);
	}

	public static Group getClassDefultGroup(Session session, Class clazz) {
		Query query = session
				.createQuery("from Group g where g.clazz=:clazz and g.name=:name");
	...
	public static SelectProblem getCurrentSelect(Session session, int id) {
		Query query = session
				.createQuery("from SelectProblem s where s.id=:id");
		query.setParameter("id", id);
		return (SelectProblem) query.list().get(0);
	}
}
